...
task usercontrol()
{
  ...
  for (int j=0; j<HISTORY; j++)
  {
	...
	while(true)
	{
			...
			//{
			//	//applied logarithmic scaling
			//	Rotate((rightStickX^2)/127);
			//}
			//else if(leftStickY != 0 && rightStickX != 0)
			//{
			//	//applied logarithmic scaling
			//	Pivot((leftStickY^2)/127, (rightStickX^2)/127);
			//}
	  ...
	  //  {
	  //  	if  (SensorValue[LeftLiftAngle] >= initialPLeft)
	  //  	{
	  //    	motor[LeftArm] = -127;
	  //  	}
	  //  	if  (SensorValue[RightLiftAngle] >= initialPRight)
	  //  	{
	  //    	motor[RightArm] = -127;
	  //    }
	  //  }
	  ...
	  //  //checks input for intake
	  //  int intakeModifier = vexRT[Btn5U] - vexRT[btn5D];

	  //  motor[LeftIntake] = intakeModifier * 127;
	  //  motor[RightIntake] = intakeModifier * 127;
...
//		}
		...
		//TESTING EVERYTHING MANUALLY HERE
/*
		if(vexRT[Btn8L] == 1)
		{
			autonomousTest();
